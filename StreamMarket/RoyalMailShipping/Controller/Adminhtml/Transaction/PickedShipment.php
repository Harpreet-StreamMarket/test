...
/**
 * Description of PickedShipment
 */
class PickedShipment extends \Magento\Backend\App\Action
{
	/**
    ...
    public function execute()
    {
        ...
        $transactionId = $this->getRequest()->getParam('transaction_id');
			...
			$send_request = $this->curlrequest->PickedCurlRequest($royalmail_shipment_id);
			...
			try {	
					if(isset($response['HttpStatusDescription']) == 'OK'){
						...
						$this->messageManager->addSuccessMessage('The Shipment number #' . $tracking_number . '-' . __(' status set picked.'));
	...
	public function saveInTransaction($tracking_number,$request_type){
			...
			$transactions = $this->transactionFactory->create()->getCollection();
                $transactions->addFieldToFilter($request_type,
                        ...
                        $_transaction->setStatus(\StreamMarket\RoyalMailShipping\Helper\Data::STATUS_PICKED)
                                ->save();
                endforeach;
		}
	}
