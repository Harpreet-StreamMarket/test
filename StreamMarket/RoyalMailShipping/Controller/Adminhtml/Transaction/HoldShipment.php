...
/**
 * Description of HoldShipment
 */
class HoldShipment extends \Magento\Backend\App\Action
{
	/**
    ...
    public function execute()
    {
        ...
        $transactionId = $this->getRequest()->getParam('transaction_id');
			...
			$send_request = $this->curlrequest->HoldCurlRequest($royalmail_shipment_id);
			...
			if(isset($response['Errors'][0])){
				...
				$response['HttpStatusDescription'] = 'OK';
			}
			try {	
					if(isset($response['HttpStatusDescription']) == 'OK'){
						...
						$this->messageManager->addSuccessMessage('The Shipment number #' . $tracking_number . '-' . __(' is on Hold.'));
						return $resultRedirect;
					}else{
						if($debug == 1){
						$writer = new \Zend_Log_Writer_Stream(BP . '/var/log/hold_shipment_error.log');
						$logger = new \Zend_Log();
	...
	public function saveInTransaction($file_name_prepare,$orderId,$shipmentId,$tracking_number,$request_type){
			...
			$transactions = $this->transactionFactory->create()->getCollection();
                $transactions->addFieldToFilter('request_type',\StreamMarket\RoyalMailShipping\Helper\Data::REQUEST_TYPE_CREATE_SHIPMENT)
				->addFieldToFilter('shipment_number',$tracking_number);
				foreach ($transactions as $_transaction):
                        $_transaction->setStatus(\StreamMarket\RoyalMailShipping\Helper\Data::STATUS_HOLD)->save();
                endforeach;
		}
    }
