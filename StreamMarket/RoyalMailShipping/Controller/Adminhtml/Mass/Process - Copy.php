...
class Process extends \Magento\Backend\App\Action implements HttpGetActionInterface
{
	...
	$data = $this->getRequest()->getParams();
	 ...
	 public function callCreateShipmentApi($shippingAddress,$billingAddress,$item_details,$orderId,$order,$service_offering,$packages)
	 {
		...
		foreach($packages as $data_items){
		$items_details = $data_items['items'];
		$package_details = $data_items['params'];
		foreach($items_details as $item_data){
		$this->apicall->callMultiPostAPI($shipmentId,$shippingAddress,$billingAddress,$item_details,$orderId,$order,$service_offering,$item_data,$package_details);
